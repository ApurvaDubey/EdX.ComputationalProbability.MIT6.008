...
def Viterbi(observations):
    ...
    for i in range(len(observations)-1):
        ...
        if i == 0:
            fm[0] = [max((e*np.array(prior))*transition[:,j]) for j in range(len(transition))]            
            tb[0] = [list((e*np.array(prior))*transition[:,r]).index(fm[0][r]) for r in range(len(transition))]
                        
        if i >= 1:
            fm[i] = [max((e*fm[i-1])*transition[:,k]) for k in range(len(transition))]
            tb[i] = [list((e*fm[i-1])*transition[:,s]).index(fm[i][s]) for s in range(len(transition))]

        fm[i] = fm[i]/sum(fm[i]) # normalize
    ...
    tb_final.append(list(e*fm[-1]).index(max(e*fm[-1]))) # last state
...
def second_best(observations):
    ...
    observations = [(1, 4), (1, 5), (1, 5), (1, 6), (0, 7), (1, 7), (3, 7), (4, 7), (4, 7), (4, 7)]
    num_time_steps = len(observations)
    estimated_hidden_states = [None] * num_time_steps # remove this
    ...
    # last estimated state
    e = np.array(emission[:,os_idx[observations[-1]]])
    second_largest_prob = second_largest(e*fm[-1])
    tb_final.append(list(e*fm[-1]).index(second_largest_prob)) # last state
