...
def Viterbi(observations):
    ...
    # YOUR CODE GOES HERE
    #
    observations = ['H','H','T','T','T']*1000
    transition = np.array([[0.75,0.25],[0.25,0.75]])
    emission = np.array([[0.5,0.5],[0.25,0.75]])
    prior = np.array([0.5,0.5])
    os_idx = {'H':0,'T':1}
    idx_hs = {0:'fair',1:'biased'}
    
    num_time_steps = len(observations)
    estimated_hidden_states = [None] * num_time_steps # remove this

    fm = np.zeros(shape=(num_time_steps-1,2))
    tb = np.zeros(shape=(num_time_steps-1,2))
